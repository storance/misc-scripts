...
def main():
    ...
    if not metadata_file.exists():
        print(f"Error: metadata.yml does not exist in \"{source_path}\"", file=sys.stderr)
        sys.exit(1)
    ...
    metadata = Metadata.from_yaml(*read_yaml_file(yaml_parser, metadata_file))

    if args.profile_path:
        profile_path = pathlib.Path(args.profile_path)
        if not profile_path.exists():
            print(f"Error: Profile path \"{profile_path}\" does not exist", file=sys.stderr)
            sys.exit(1)
    else:
        profile_path = source_path / 'profiles' / pathlib.Path(args.profile).with_suffix('.yml')
        if not profile_path.exists():
            print(f"Error: Profile \"{args.profile}\" does not exist in \"{source_path}/profiles\"", file=sys.stderr)
            sys.exit(1)

    profile = Profile.from_yaml(*read_yaml_file(yaml_parser, profile_path), metadata=metadata)
...
def read_yaml_file(yaml_parser: YAML, file: pathlib.Path) -> tuple[dict, Location]:
    try:
...
def sync_roms(source_path: pathlib.Path,
    ...
    if dry_run:
        ...
        run_copy_sync(to_copy, threads, chunk_size, total_size)
        ...
        for glob_pattern in rom_folder_config.rom_folder.includes:
            for file in scan_dir.glob(glob_pattern):
                ...
                if not dest.is_relative_to(dest_path):
                    raise ValueError(
                        f"Rom destination path \"{dest}\" is not relative to the configured destination folder.")
    ...
    for profile_rom_folder in profile.rom_folders:
        ...
        for root, dirnames, filenames in profile_rom_folder.destination.walk():
            ...
            if not profile_rom_folder.folder_per_game or nesting_level >= 1:
                dirnames.clear()
            elif profile_rom_folder.folder_per_game and nesting_level == 0:
...
def is_interested_rom(file: pathlib.Path, rom_folder_config: ProfileRomFolderConfig) -> bool:
    if any(file.full_match(exclude) for exclude in rom_folder_config.rom_folder.excludes):
        return False
...
def print_dry_run_summary(files_to_delete: set[pathlib.Path],
    ...
    if extra_space > 0:
        print(
            f"DRY RUN: Space Required: {humanfriendly.format_size(extra_space, binary=True)}")
    elif extra_space < 0:
        print(
            f"DRY RUN: Space Freed: {humanfriendly.format_size(-extra_space, binary=True)}")
...
def run_copy_sync(copy_tasks: set[CopyTask], threads: int, chunk_size: int, total_size: int):
    if len(copy_tasks) == 0:
        return
    ...
    with tqdm(
        ...
        if threads == 1:
            for task in copy_tasks:
                copy_file_with_progress(task, 1, file_progress, bytes_progress, chunk_size)
        else:
            row_pool = Queue()
            ...
            with ThreadPoolExecutor(max_workers=threads) as executor:
                futures = [executor.submit(copy_file_with_progress_thread,
                                           task,
                                           row_pool,
                                           file_progress,
                                           bytes_progress,
                                           chunk_size) for task in copy_tasks]
