...
def main():
    ...
    parser.add_argument('-e', '--ext', action="append", default=[], help="Filter to only include the specified file extension.")
    ...
    for file in args.input_directory.glob(glob_pattern):
        ...
        if sha1_file.exists() and not args.overwrite:
            print(f"SHA1 hash file exists for \"{file}\". Skipping...")
            continue
